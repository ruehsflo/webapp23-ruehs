/**
 * @fileOverview  View code of UI for managing Persondata
 * @author Gerd Wagner
 */
/***************************************************************
 ...
 document.getElementById("RetrieveAndListAll").addEventListener("click",
     function () {
       ...
       for (const key of Object.keys(Person.instances)) {
         ...
         const listElPlayed = createListFromMap(person.playedMovies, "movieId");
         row.insertCell().appendChild(listElPlayed);

         const listElDirected = createListFromMap(person.directedMovies, "movieId");
         row.insertCell().appendChild(listElDirected);
       }
