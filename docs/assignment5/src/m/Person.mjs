...
Person.destroy = function (personId) {
  ...
  for (const key of Object.keys( Movie.instances)) {
    ...
    if (movie.actors[personId]) delete movie.actors[personId]; // if person is an actor it has to be removed from movies
    if (movie.director?.personId === personId) {  //if person was a director it has to be deleted
